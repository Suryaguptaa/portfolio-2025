...
export default function Home() {
    ...
    elements.forEach((el) => {
      gsap.fromTo(el,
        { y: 50, opacity: 0 },
        {
          ...
          scrollTrigger: {
            trigger: el,
            start: "top 85%", // Start animation when element is 85% down screen
          }
        }
      );
    });
  ...
  return (
    <main className="min-h-screen bg-background text-paper overflow-x-hidden">
      ...
      <section className="h-screen flex flex-col justify-center px-4 md:px-8 pt-20 relative overflow-hidden">
        ...
        <div className="absolute inset-0 w-full h-full z-0">
          <video
            ...
            loop
            muted
            playsInline        // <--- FIX FOR IPHONE
            preload="none"     // <--- HELPS LOAD SPEED
            poster="/poster.jpg" // <--- FALLBACK IMAGE IF VIDEO FAILS
          />
          {/* Dark Overlay (Ensures text is readable) */}
          <div className="absolute inset-0 bg-black/60"></div>
        </div>
        {/* --- THE VIDEO BACKGROUND END --- */}

        {/* Content (Z-Index 10 puts it ON TOP of video) */}
      ...
      {/* SECTION 2: MARQUEE (Moving Text Strip) */}
      ...
      {/* SECTION 4: SERVICES (Dynamic List) */}
